...
with open(filename.labelcsv, 'r', encoding='utf-8') as f:    
    reader = csv.reader(f)
    csvList = list(reader)  
    csvList = csvList[1:]
N.data = len(csvList)

pNames = np.array([row[0] for row in csvList])
np.save(os.path.join(directory.dataDirProcessed, "pNames.npy"), pNames)
...
for ii in xrange(N.data):    
  ...
  import os
  filename.currentTrainingFile = os.path.join(directory.dataDir, csvList[ii][0])
  ...
  if csvList[ii][0] in dupes:
    ...
    if args.downsampleImage != -1.0:
      ...
      stftImage = stftImage.resize(
        [int(s * args.downsampleImage) for s in stftImage.size],
        resample=Image.BICUBIC
      )
      stftImage = np.array(stftImage)
