  ...
  def show_layer_parameter_percentages(self):
    ...
    for m in self.modules():        
        if isinstance(m, Net):            
            ...
            print ('Module: ' + m.__class__.__name__ + ' params: %2.2f')
            print(100.0*N_currentParams / float(self.N_dnnParameters))

    print ('\n')    
    print ('Total number of trainable parameters: %5d\n')
    print(self.N_dnnParameters)
    assert(self.N_runningParams == self.N_dnnParameters)        
    

  def initialize_layers(self):
    
    print ('Initializing layers via: ' + self.weightsInitsScheme + ', biases to: %2.2f')
    ...
    for m in self.modules():        
        if isinstance(m, nn.Conv2d) or isinstance(m, nn.Linear):
            ...
            # Print the layer:        
            print ('Module: ' +  m.__class__.__name__ + ' FanIn: %1d')
