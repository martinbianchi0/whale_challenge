...
def save_gan_models(gen, disc, path_disc="saved_models/gan/disc.pth", path_gen="saved_models/gan/gen.pth"):
...
def load_gan_models(device, z_dim=100, path_disc=r"saved_models\gan\disc.pth", path_gen=r"saved_models\gan\gen.pth"):

    gen = SpecGANGenerator(z_dim).to(device)
    disc = SpecGANDiscriminator().to(device)
